...
ki = LINETCR.LINE()
#ki.login(qr=True)
ki.login(token='isi token anda')
ki.loginResult()

kk = LINETCR.LINE()
#kk.login(qr=True)
kk.login(token='isi token anda')
kk.loginResult()

kc = LINETCR.LINE()
#kc.login(qr=True)
kc.login(token='isi token anda')
kc.loginResult()

ks = LINETCR.LINE()
#kr.login(qr=True)
ks.login(token='isi token anda')
ks.loginResult()

satpam = LINETCR.LINE() # 
satpam.login(token='isi token anda')#6
...
#----------------------------------------------------------------------------                
            ...
            elif msg.text in ["3rename "]:
              if msg.from_ in admin:
                string = msg.text.replace("3rename ","")
                if len(string.decode('utf-8')) <= 20:
                    profile_B = kk.getProfile()
                    profile_B.displayName = string
                    kk.updateProfile(profile_B)
                    kk.sendText(msg.to,"name " + string + " done")
            elif msg.text in ["4rename "]:
              if msg.from_ in admin:
                string = msg.text.replace("4rename ","")
                if len(string.decode('utf-8')) <= 20:
                    profile_B = kc.getProfile()
                    profile_B.displayName = string
                    kc.updateProfile(profile_B)
                    kc.sendText(msg.to,"name " + string + " done")
            elif msg.text in ["5rename "]:
              if msg.from_ in admin:
                string = msg.text.replace("5rename ","")
                if len(string.decode('utf-8')) <= 20:
...
#-----------------------------------------------
       ...
       #-------------Fungsi Respon Start---------------------#
              ...
              if msg.from_ in admin:
                kk.sendText(msg.to,"Tukang Ngintip On")
                ki.sendText(msg.to,"Tukang Nikung On")
                cl.sendText(msg.to,"Tukang Mojok On")
                kc.sendText(msg.to,"Tukang Baper On")
                ks.sendText(msg.to,"Tukang Ngocok On")
